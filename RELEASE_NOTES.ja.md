...
## v1.0.2 (2026-07-01) — 結果グリッド：列幅調節・表示/非表示・ピン留め

結果グリッドの日常操作を快適にするリリースです。結果グリッドの列を直接操作できるようにしました——幅を変え、不要な列を隠し、必要な列を左に固定できます。設定はすべてタブ単位・セッション限定です（再起動をまたいだ永続化はありません）。

### ドラッグで列幅を調節 (#59)
- 列の境界をドラッグして幅を設定、または境界を**ダブルクリック**すると自動フィット幅に戻ります。幅は列名をキーに保持されるため、ソートや並び替えをしても維持されます。

### 列の表示/非表示 (#60)
- ツールバーの **Columns ▾** パネルで列ごとに表示/非表示を切り替え、**すべて表示**で一括再表示できます。
- 列ヘッダの右クリックから **この列を非表示 / すべて表示** も可能です。
- 最後に残った可視列は非表示にできません（ストアと UI の両層でガード）。グリッドが空になることはありません。

### 列を左にピン留め（固定列） (#60)
- ヘッダ右クリックメニュー、または Columns パネルの 📌 ボタンで列をピン留めできます。ピン列は左端に寄せられ、横スクロールしても `position: sticky` で固定表示され、手前のピン列をリサイズするとピン位置も追従します。
- 詳細ペイン・CSV/TSV エクスポート・行コピーは全列・元の順序のままです——非表示とピン留めは画面上のグリッドにのみ影響します。

---

## v1.0.1 (2026-06-28) — UX 細部修正：DML フィードバック・NULL 挿入・キーボード編集・接続ガード

日常操作で目につく 4 つの小さな摩擦を解消するパッチです。

### DML / DDL 結果の影響行数表示 (U5)
- UPDATE・INSERT・DELETE・DDL 文の実行後、結果ペインとステータスバーに「✓ N 行が更新されました」を表示するようにしました。これまでは誤解を招く「結果なし（0 行）」と表示されていました。

### INSERT 行エディタへの NULL ボタン追加 (U8)
- 新しい行を追加するとき、各セルのエディタに **NULL** ボタンを追加しました。INSERT コミット前に、空文字や DB デフォルトとは区別した明示的な `NULL` をセットできます。

### Enter キーによる編集開始と主キーなしテーブルへの読み取り専用注記 (U9)
- 行を選択した状態で **Enter** キーを押すと、最初の編集可能カラムの編集を開始するようにしました。一般的なグリッドアプリと同様の操作感になります。
- 主キーのないテーブルでは、なぜセル編集が無効になっているかを説明する読み取り専用バナーを表示するようにしました（これまでは無音で無視されていました）。

### 接続の二重発火ガード
- 接続処理が進行中の間、Connect ボタンとダブルクリックを無効化しました。誤って 2 つ目の接続試行が同時にトリガされるのを防ぎます。

---

## v1.0.0 (2026-06-24) — Reef デビュー：オープンソース化・国際化・オーバーレイの a11y

新しい名前 **Reef**（旧 Table++）としての最初のリリースであり、プロジェクトの**オープンソース化**のお披露目です。リネームと併せて、長らく後回しにしていた次バッチ——UI の国際化、タブ操作のキーボードショートカット、モーダル・コンテキストメニューのアクセシビリティ——に、開発基盤（Biome）と挙動を変えない内部クリーンアップをまとめました。

### Reef へリブランド、オープンソース化 (#48, #49, #50)
- アプリとプロジェクトを **Table++ → Reef** にリネームし、README とノートを英語化、ソースを **MIT ライセンス**で公開、GitHub リポジトリを **public** にしました。アプリアイコンを Reef ブランドに刷新し、リポジトリ URL も更新しています（`docs/superpowers` の作業ノートは公開ツリーから除外）。

### 国際化 — 英語 / 日本語 (#51)
- UI 全体を**国際化（英語 / 日本語）**しました。軽量な自作カタログ（`src/shared/i18n`、**新規依存ゼロ**）を採用し、既定は英語、初回起動時にシステム言語を自動判定、設定モーダルから言語を切り替えられます。レンダラとネイティブメニューの両方が選択に追従します。

### タブ操作のキーボードショートカット (#53)
- ワークスペースのタブをキーボードで操作できるようにしました：**⌘T** で新規 SQL タブ、**⌘W** でアクティブタブを閉じる（閉じるタブが無ければウィンドウを閉じる挙動へフォールバック）、**⌘1〜8** で N 番目／**⌘9** で末尾のタブへ、**⌘⇧] / ⌘⇧[** で次／前のタブへ巡回。New Tab・Close Tab はメニューバーにも表示されます。

### モーダル・コンテキストメニューの a11y (#55)
- モーダルと右クリックメニューを、共通のアクセシブルなコンポーネントに作り替えました。モーダルは **dialog セマンティクス**（`role="dialog"` / `aria-modal`）、**Esc クローズ**、**フォーカストラップ**（Tab / Shift+Tab がダイアログ内で循環）、初期フォーカス、閉じた後の**フォーカス復帰**を備えます（インポート実行中は閉じません）。
- コンテキストメニューは**画面端でクランプ／フリップ**（右下隅でも見切れない）し、**キーボード操作**（↑/↓ で移動、Enter で実行、Esc で閉じる）に対応。`menu` / `menuitem` ロールを付与し、メニューのキー操作が背後のグリッドへ漏れないよう分離しています。

### 開発基盤：Biome (#52)
- Lint / Format に **Biome** を導入し、**CI に lint ジョブを追加**しました。スタイルの乱れや一定のバグを、push / PR ごとに自動で弾きます。

### 内部クリーンアップ (#54)
- 挙動を変えない低リスクな負債解消：レンダラの `window.api` 型を preload の単一の `Api` 定義から取得するように一本化、テーブル閲覧のステージリセット（7 箇所）を 1 つのヘルパーに集約、識別子エスケープの `quoteIdent` を共有モジュールに統一。

### 今後の予定（v0.5 以降）
- 外部キージャンプ / エクスポート形式の追加 / 設定画面 / 型対応のセル編集 / コード署名・notarization ほか
