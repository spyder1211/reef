...
- **（最終レビュー指摘の追加確認）矢印キーで最上段の行をアクティブにしたとき、その行が sticky ヘッダの下に隠れない**（ArrowUp/ArrowDown 両方向、上端・下端境界で行が完全に見える）。
- **（最終レビュー指摘の追加確認）セルをダブルクリック編集したとき、input／NULL ボタンが固定行高（25px）の行からはみ出して隣行に被らない**。
